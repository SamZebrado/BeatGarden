...
export class ModeSelectView {
  ...
  show(): void {
    ...
    page.innerHTML = `
      <div style="display:flex;justify-content:flex-end"><button data-role="language" aria-label="${languageTargetAction()}" title="${languageTargetAction()}" style="padding:10px 16px;border-radius:999px;border:1px solid #537496;background:#10253a;color:#fff">${languageTargetLabel()}</button></div>
      ...
      <footer style="margin-top:26px;padding:8px 0;text-align:center"><a href="https://samzebrado.github.io/IslandSlonelyFall/" target="_blank" rel="noopener noreferrer" style="display:inline-block;color:#a9c9df;font-size:14px;text-decoration:none">${t('mode.moreBySam')}</a></footer>`;
